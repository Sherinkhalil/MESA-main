...
def runners_data():
    ...
    runners_id = []
    for line in lines:
        if line != "\n":    # If an empty line is read, it will be skipped
...
def users_venue(races_location, runners_id):
    ...
    for i in range(len(runners_id)):
        ...
        if time_taken_for_runner == 0: # Looks weird, might need changes
...
def displaying_runners_who_have_not_won_at_least_one_race(races_location, runners_name, runners_id):
    pass
